...
let
  chrome = pkgs.google-chrome.override {
    commandLineArgs = [
      ...
      "--user-data-dir=/Users/kyungrok.chung/store/google-chrome"
    ];
  };

  chromeRenamed = pkgs.runCommandLocal "google-chrome-renamed" { } ''
    mkdir -p "$out/Applications"
    cp -R "${chrome}/Applications/Google Chrome.app" "$out/Applications/Chrome.app"
  '';
in
{
  imports = [
  ...
  home.activation.setWallpaper = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    run ${pkgs.desktoppr}/bin/desktoppr ${./assets/firewatch-dark-minimal-5k-2560x1600.jpg}
  '';
  ...
  launchd.agents = {
    sockd = {
      ...
      config = {
        KeepAlive = true;
        RunAtLoad = true;
        Program = "/run/current-system/sw/bin/sleep";
        ProgramArguments = [
          "24h"
        ];
        # StartInterval = 21600;
        # EnvironmentVariables.PATH = "${pkgs.restic}/bin:/usr/bin";
        StandardErrorPath = "/Users/kyungrok.chung/tmp/sockd.stderr"; # these lines are how you debug stuff with launchd
        StandardOutPath = "/Users/kyungrok.chung/tmp/sockd.stdout";
      };
    };
  };
  ...
  home.packages = [
    ...
    (pkgs.google-chrome.override {
      ...
      commandLineArgs = [
        ...
        "--ozone-platform=x11"
        "--remote-debugging-port=9222"
        "--user-data-dir=/Users/kyungrok.chung/store/chrome/main"
