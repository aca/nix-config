...
# secrets = "wer";
{
  ...
  networking.hosts."192.168.195.212" = [ "tsvm" ];
  ...
  fileSystems."/mnt/archive-0" = {
    device = "192.168.0.177:/mnt/archive-0";
  ...
  imports = [
    "${inputs.nixpkgs-unstable}/nixos/modules/services/system/nohang.nix"
    ...
    (import "${inputs.internal}/work.nix" { ztInterface = "zt6jyqdca7"; })
  ...
  environment.systemPackages =
    ...
    ++ [
      ...
      (vivaldi.override {
        commandLineArgs = [
          ...
          "--user-data-dir=/home/rok/store/vivaldi/main"
          # NOTES: ozone-platform=wayland fcitx win+space not work
        ];
      })
    ...
    ++ [
      (pkgs.writeShellScriptBin "pbpaste" ''if [ "$WAYLAND_DISPLAY" != "" ]; then exec wl-paste --no-newline; else exec xsel --clipboard --output; fi'')
      (pkgs.writeShellScriptBin "pbcopy" ''if [ "$WAYLAND_DISPLAY" != "" ]; then exec wl-copy; else exec xsel --clipboard --input; fi'')
