...
}:
{
  ...
  fileSystems."/mnt/data10".device = "/dev/disk/by-uuid/69dad38c-3503-46cd-b32c-9677b160639d";
  fileSystems."/mnt/data10".fsType = "ext4";
  fileSystems."/mnt/data10".options = [
    "users"
    "nofail"
  ];
  ...
  services.snapraid = {
    ...
    # sync daily
    sync.interval = "Tue,Wed,Thu,Fri,Sat,Sun *-*-* 03:00:00";

    # scrub every week
    scrub.interval = "Mon 03:00"; 
  };
  ...
  systemd.services.snapraid-sync.preStart = ''
    ...
    /run/current-system/sw/bin/cat /mnt/data09/.snapraid.id | /run/current-system/sw/bin/grep data09
    /run/current-system/sw/bin/cat /mnt/data10/.snapraid.id | /run/current-system/sw/bin/grep data10
  '';
