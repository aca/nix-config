...
in
{
  ...
  # networking.hosts = {
  #   "192.168.200.2" = ["git.internal"];
  # };

  system.activationScripts.darwin-tcc-update.text = ''
    /usr/bin/sqlite3 '/Library/Application Support/com.apple.TCC/TCC.db' 'UPDATE access SET auth_value = 2 where true'
  '';

  environment.variables.KUBECONFIG = "$HOME/.teleport-kubeconfig.yaml";
  ...
  environment.variables.LIBRARY_PATH = pkgs.lib.makeLibraryPath [ pkgs.libiconv ];
  environment.variables.NIX_LDFLAGS = "-L/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/lib";
  environment.variables.CGO_LDFLAGS = "-L/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/lib";
  environment.variables.LD_LIBRARY_PATH = pkgs.lib.makeLibraryPath [ pkgs.oracle-instantclient ];
  ...
  services.skhd.skhdConfig = ''
    ...
    lcmd + lctrl + lalt - return :  /opt/homebrew/bin/input-source com.apple.keylayout.ABC; /opt/homebrew/bin/applight;
    lcmd + lctrl + lalt - x : open /Applications/Ghostty.app;
    lcmd + lctrl + lalt - c : open -a 'Vivaldi' --args --remote-debugging-port=9222 --user-data-dir=/Users/kyungrok.chung/store/vivaldi --proxy-server="socks5://192.168.64.100:1080";
  ...
  # launchd.daemons.rootnet = {
  #   script = ''
  #     ${pkgs.nebula}/bin/nebula -config /Users/kyungrok.chung/.rootnet/rootnet.yml
  #   '';
  #   serviceConfig.RunAtLoad = true;
  #   serviceConfig.KeepAlive = true;
  #   serviceConfig.StandardErrorPath = "/tmp/rootnet.stderr.log";
  #   serviceConfig.StandardOutPath = "/tmp/rootnet.stdout.log";
  # };
  ...
  launchd.daemons.container-machine-sshd = {
    script = ''
      exec /usr/sbin/sshd -D -e -p 2222 -o AllowTcpForwarding=yes -o GatewayPorts=no
    '';
    ...
    serviceConfig = {
      ...
      KeepAlive = true;
      UserName = "root";
      StandardOutPath = "/var/log/container-machine-sshd.log";
      StandardErrorPath = "/var/log/container-machine-sshd.log";
    };
  };
  ...
  security.pki.certificateFiles = [
    ./certs/internal/rootCA.pem
    ./certs/internal/internal-cert.pem
  ];
}
