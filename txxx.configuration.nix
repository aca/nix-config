...
kyungrok.chung ALL = (ALL) ALL
  ...
  services.skhd.skhdConfig = ''
    ...
    lcmd + lctrl - y : /run/current-system/sw/bin/work.antisleep
    lcmd + lctrl + shift - y : /usr/bin/pkill -f work.antisleep
    lcmd + lctrl - x : open /Applications/Ghostty.app;
    lcmd + lctrl - 0x32 : open /Applications/Ghostty.app;
    # lcmd + lctrl - c : open -a 'Firefox Developer Edition';
    # lcmd + lctrl - c : /Applications/Vivaldi.app/Contents/MacOS/Vivaldi --remote-debugging-port=9222;
    lcmd + lctrl - c : open -a 'Vivaldi' --args --remote-debugging-port=9222 --remote-debugging-host=0.0.0.0;
    ...
    # terminal (backtick)
    # lcmd + lctrl - c : open ${pkgs.firefox-devedition-bin};
    # lcmd + lctrl - c : /Users/rok/bin/neovide; skhd -k "escape"
