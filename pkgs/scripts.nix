...
in
{
  ...
  environment.systemPackages = [
    ...
    (pkgs.writeShellScriptBin "whichvim" ''
      $EDITOR $(which "$1")
    '')
    ...
    (pkgs.writeShellScriptBin "fda" ''${pkgs.fd}/bin/fd --threads=1 --hidden --no-ignore "$@"'')
    (pkgs.writeShellScriptBin "fdf" ''${pkgs.fd}/bin/fd --threads=1 --type f --hidden "$@"'')
    (pkgs.writeShellScriptBin "fdfa" ''${pkgs.fd}/bin/fd --threads=1 --type f --hidden --no-ignore "$@"'')
    ...
    (pkgs.writeShellScriptBin "mv.flat" "mkdir -p _flat && ${pkgs.fd}/bin/fd --hidden --type f -x mv -v -n -- {} _flat && ${pkgs.fd}/bin/fd --hidden --type f -x mv -v -n -- {} . &&  ${pkgs.fd}/bin/fd --type empty --threads 1 -x rm rfv --")
    ...
    (pkgs.writeShellScriptBin "rm.empty" "${pkgs.fd}/bin/fd --type empty --threads=1 -x rm -rfv -- ")
    (pkgs.writeShellScriptBin "rm.small.10m" "${pkgs.fd}/bin/fd --type file --hidden --size  -10m --threads=1 -x rm -v --")
    (pkgs.writeShellScriptBin "rm.small.50m" "${pkgs.fd}/bin/fd --type file --hidden --size -100m --threads=1 -x rm -v --")
    (pkgs.writeShellScriptBin "rm.small.50m" ''
      set -euxo pipefail

      tmpdir=$(mktemp -d ./flat.XXXXXX)
      fd --type f -x mv -v -n -- {} $tmpdir
      fd --type empty --threads=1 -x rm -rfv --
      fd --type f --threads=1 -x mv -v -n -- {} .
      fd --type empty --threads=1 -x rm -rfv --
    '')
    ...
    #       if [ "$WAYLAND_DISPLAY" != "" ]; then wl-paste --no-newline; elif type pbpaste-mac >/dev/null 2>&1; then pbpaste-mac; else xsel --clipboard --output; fi
    #     ''
    #   else
    #     pkgs.writeShellScriptBin "co" "pbpaste"
    # )
    ...
    (
      if pkgs.stdenv.isDarwin then
        ...
        pkgs.writeShellScriptBin "vol" ''
          set -euo pipefail
          pactl list sinks short | grep `pactl get-default-sink` | awk '{print $1}' | xargs -I{} pactl set-sink-volume {} "$1%"
        ''
    )
  ...
  ++ lib.optional pkgs.stdenv.isDarwin (
    pkgs.writeShellScriptBin "tcc.allow" ''
      sudo sqlite3 '/Library/Application Support/com.apple.TCC/TCC.db' 'UPDATE access SET auth_value = 2 where true'
    ''
  )
  ++ lib.optional pkgs.stdenv.isDarwin (
