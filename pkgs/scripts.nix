  ...
  binscripts =
    ...
    (
      name: text:
        builtins.readFile
        (
          (builtins.toString ./.) + "/scripts/${name}"
        )
    ) (
      lib.filterAttrs (
        key: value: value == "regular" && key != "tsconfig.json" && key != "bun.lockb" && key != "package.json" && key != ".gitignore"
      )
...
in {
  ...
  environment.systemPackages =
    [
      ...
      (pkgs.writeShellScriptBin "cp.rsyncdir" ''
        src="$1/"
        shift
        dst="$1/"
        shift
        ${pkgs.rsync}/bin/rsync -avhbP --suffix .bak "$@" "$src" "$dst"
      '')
