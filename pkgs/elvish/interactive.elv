...
set edit:command-abbr['ju'] = 'sudo journalctl -u'
set edit:command-abbr['juu'] = 'journalctl --user -u'
...
set edit:command-abbr['got'] = 'go test -v -run '
...
set edit:before-readline = [
  ...
  {
    var osc7-uri = (printf "file://%s%s" (platform:hostname) $pwd)
    printf "\e]7;%s\e\\" $osc7-uri > /dev/tty
    if (has-env TMUX) {
      printf "\ePtmux;\e\e]7;%s\a\e\\" $osc7-uri > /dev/tty
    }
  }
]
