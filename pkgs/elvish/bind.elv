...
use readline-binding # emacs mode
...
fn fzf_cd {||
    cd (try { fd --hidden --type d  --max-depth 9 --no-ignore -0 | fzf --read0 --height=50% --min-height 14 } catch { echo '.' } )
    edit:redraw &full=$true
  # try {
  # } catch {
  #   edit:redraw &full=$true
...
fn fzf_cd_src { try { cd (src.dir) edit:redraw &full=$true } catch { edit:redraw &full=$true return } }

set edit:insert:binding[Ctrl-S] = { fzf_cd_src > /dev/tty 2>&1 }
# set edit:insert:binding[Ctrl-B] = { edit:move-dot-left-word }
set edit:insert:binding[Ctrl-E] = { edit:move-dot-eol }
set edit:insert:binding[Ctrl-D] = { edit:location:start }
set edit:insert:binding[Ctrl-Q] = {
    if (has-env "TMUX") {
        tmux send-keys -R
        nop ?(tmux clear-history 2>/dev/null)
...
# set edit:insert:binding[Ctrl-E] = { tput reset > /dev/tty > 2>&1 }
# set edit:insert:binding[Ctrl-E] = { use tty; tty:hide-cursor; clear > /dev/tty ; edit:redraw &full ; tty:show-cursor }
...
#     edit:replace-input ( printf "cq -- elvish -c %q" (put $edit:current-command | to-lines | sh -c 'x=$(cat -); printf "%q" "$x"'))
#     edit:smart-enter
#   }
# }
...
set edit:insert:binding[Ctrl-G] = { 
    edit:replace-input ( printf "go run ." )
    edit:smart-enter
}
