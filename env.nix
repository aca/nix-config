{
  ...
  environment.extraInit =
    ...
    # lang:js
    + ''
      export NODE_OPTIONS="--experimental-fetch --experimental-top-level-await --experimental-modules --no-warnings"
      export NPM_CONFIG_GLOBALCONFIG=~/.npmrc.global
    ...
    + ''
      # FZF
      export FZF_DEFAULT_COMMAND='fd -L --hidden --type f --type symlink'
      export FZF_DEFAULT_OPTS='--min-height 15 --reverse --color "gutter:-1" --info=inline --no-scrollbar --no-separator --cycle -m --bind ctrl-a:toggle-all --bind ctrl-n:down --bind ctrl-d:page-down --bind ctrl-u:page-up --bind ctrl-p:up --bind ctrl-w:toggle-preview --prompt "» " --preview "bat {}" --preview-window "hidden"'
      export FZF_ALT_C_COMMAND='fd --hidden --type d --max-depth 10 --no-ignore'
    ''
    # tool:libvirt
    + ''
