  ...
  environment.variables =
    {
      ...
      "FZF_DEFAULT_COMMAND" = "fd -L --hidden --type f --type symlink";
      "FZF_DEFAULT_OPTS" = "--min-height 15 --reverse --color gutter:-1 --info=inline --no-scrollbar --no-separator --cycle -m --bind ctrl-a:toggle-all --bind ctrl-n:down --bind ctrl-d:page-down --bind ctrl-u:page-up --bind ctrl-p:up --bind ctrl-w:toggle-preview --prompt '» ' --preview 'bat {}' --preview-window hidden";
      "FZF_ALT_C_COMMAND" = "fd --hidden --type d --max-depth 10 --no-ignore";
    }
    // (
      if pkgs.stdenv.isDarwin
      then {
        PATH = "$HOME/bin/x:$HOME/bin:$HOME/.cargo/bin:$PATH";
      }
      else {
        PATH = "$HOME/src/go.googlesource.com/go/bin:$HOME/bin/x:$HOME/bin:/nix/var/nix/profiles/default/bin:/run/current-system/sw/bin:/opt/homebrew/bin:$HOME/.cargo/bin:$PATH";
